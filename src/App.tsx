...
function App() {
  ...
  const handleKeyPress = (e: React.KeyboardEvent) => {
    const key = e.key;
    if (key === words.currentString) { // input is correct
      setInputWrong("");
      setWords({
    ...
    } else if (key === "Backspace") { // if backspace pressed
      if (inputWrong) { // if there's already a string of wrong pressed keys
        setInputWrong(inputWrong.substr(0, inputWrong.length - 1));
        setWords({
          prevString: words.prevString,
          currentString: words.currentString.substring(0, Math.max(words.currentString.length - 1, 1)),
          nextString: inputWrong.length === 1 ? words.nextString : words.currentString.substr(-1) + words.nextString,
        });
      } else { // if you're just trying to erase a correct key
        setWords({
          prevString: words.prevString.substring( 0, words.prevString.length - 1),
          currentString: words.prevString.substr(-1),
          nextString: words.currentString + words.nextString,
        });
      }
    } else if (key.match("^[a-zA-Z]$")) { // if the key pressed is incorrect
  ...
  return (
    ...
    >
      <div className="w-3/4 max-w-4xl text-justify bg-gray-800 rounded-lg p-5">
